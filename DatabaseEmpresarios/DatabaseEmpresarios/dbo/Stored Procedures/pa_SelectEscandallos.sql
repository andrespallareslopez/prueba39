...
CREATE PROCEDURE [dbo].[pa_SelectEscandallos]
   @CodProducto_Num int
AS
...
select * into #TempEscandallo from (select Cast(0 as bit) as CalculoPorPeso,Cast(0 as bit) as CalculoPorRacion,Null as IDClasePadre,cast(1.000 as decimal(18,4)) as Factor,* from gesescandallo_Num where codproducto_Num=@codproducto_Num ) ll
...
(select top 1 precio from #tempgescatalogo b where b.codarticulo=(select top 1 CodArticulo from #ConsPedidoArt b where b.codclase=a.codclase order by  fechapedido desc)) as precioCompra,
...
from #ConsArt a inner join #tempEscandallo b on a.codclase=b.codclase) jj

select @TieneContenidoUnidad=(select count(*) from #ConsESc where codproducto_num=@Codproducto_num and Contenidounidad=0)
set nocount off
...
(select N from #ConsPedidoArt b where a.codarticulo=b.codArticulo) as N 
 from #ConsEsc a where codproducto_num=@CodProducto_num order by a.codproducto_num
end
else
begin
select a.*,
